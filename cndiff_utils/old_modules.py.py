...
class ClassificationCondition(nn.Module):
    def __init__(self, config):
        ...
        self.net = nn.Sequential(
            ...
            nn.GELU(),  # Replacing ReLU with GELU
            nn.Conv1d(
                config.hidden_dim,
                config.hidden_dim,
                kernel_size=3,
        ...
        self.residual = nn.Conv1d(
            config.feature_dim, config.hidden_dim, kernel_size=1
        )  # Match dimensions if needed

    def forward(self, x):
        x_ = x.permute(0, 2, 1)  # (B, H, T)
        out = self.net(x_) + self.residual(x_)  # Residual connection
        out = out.permute(0, 2, 1)  # Back to (B, T, H)
        return self.project(out) + x  # (B, T, feature_dim)
