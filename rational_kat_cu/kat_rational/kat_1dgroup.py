...
class rational_1dgroup(torch.autograd.Function):
    ...
    def forward(ctx, input, weight_numerator, weight_denominator, group):
        ...
        ctx.group = group
        x = rational_kat_cu.rational_fwd_1dgroup(
    ...
    def backward(ctx, grad_output):
        ...
        group = ctx.group
        d_input, d_weight_numerator, d_weight_denominator = (
            rational_kat_cu.rational_bwd_1dgroup(
    ...
    def initialize(self, mode="gelu"):
        ...
        try:
            ...
            self.weight_numerator = nn.Parameter(
                weight_numerator.float(), requires_grad=True
            )
            self.weight_denominator = nn.Parameter(
                weight_denominator.float(), requires_grad=True
            )
