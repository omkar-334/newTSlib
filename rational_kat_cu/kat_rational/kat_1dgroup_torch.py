...
def Rational_CUDA_A_1DGroup(x, weight_numerator, weight_denominator, group):
    ...
    z = (
        x.view(B, L, group, D_per_group).permute(2, 0, 1, 3).contiguous()
    )  # Shape: (group, B, L, D_per_group)
    ...
    xps = _get_xps(
        z, len_num, len_deno
    )  # Should output shape: (group, B * L * D_per_group, max(len_num, len_deno))

    # Compute numerator as a dot product of powers of z and weights
    numerator = torch.bmm(weight_numerator.unsqueeze(1), xps).squeeze(
        1
    )  # Shape: (group, B * L * D_per_group)
    ...
    expanded_dw = torch.cat(
        [
            torch.ones(
                group, 1, device=device
            ),  # 1 for the constant term of denominator
            weight_denominator,
            torch.zeros(
    ...
    denominator = torch.bmm(expanded_dw.abs().unsqueeze(1), xps).squeeze(
        1
    )  # Shape: (group, B * L * D_per_group)
    ...
    result = (
        result.view(group, B, L, D_per_group).permute(1, 2, 0, 3).contiguous()
    )  # Shape: (B, L, group, D_per_group)
...
class KAT_Group_Torch(nn.Module):
    ...
    def initialize(self, mode="gelu"):
        ...
        try:
            ...
            weight_denominator = torch.cat([weight_denominator] * self.num_groups).view(
                self.num_groups, -1
            )

            self.weight_numerator = nn.Parameter(
                torch.FloatTensor(weight_numerator), requires_grad=True
            )
            self.weight_denominator = nn.Parameter(
                torch.FloatTensor(weight_denominator), requires_grad=True
            )
    ...
    def forward(self, input):
        ...
        """
        assert input.dim() == 3, (
            "Input tensor must be 3D. Of size (batch, length, channels)."
        )
        ...
        return Rational_CUDA_A_1DGroup(
            input, weight_numerator, self.weight_denominator, self.num_groups
        )
